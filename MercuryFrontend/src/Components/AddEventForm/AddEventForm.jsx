...
export default function AddEventForm({ availableServices }) {
  ...
  const handleFirstNameChange = (name) =>
    updateAppointment((draft) => {
      draft.customer.firstName = name.target.value;
    });
  const handleLastNameChange = (name) =>
    updateAppointment((draft) => {
      draft.customer.lastName = name.target.value;
    });
  const handlePhoneChange = (phone) =>
    updateAppointment((draft) => {
      draft.customer.phone = phone.target.value;
    });
  const handleEmailChange = (email) =>
    updateAppointment((draft) => {
      draft.customer.email = email.target.value;
    });
  const handleCityChange = (city) =>
    updateAppointment((draft) => {
      draft.address.city = city.target.value;
    });
  const handleStreetChange = (street) =>
    updateAppointment((draft) => {
      draft.address.street = street.target.value;
    });
  const handlePostalCodeChange = (postalCode) =>
    updateAppointment((draft) => {
      draft.address.postalCode = postalCode.target.value;
    });
  const handleDateChange = (date) =>
    updateAppointment((draft) => {
      draft.date = date.target.value;
    });
  const handleServicesChange = (services) =>
    updateAppointment((draft) => {
      draft.selectedServices = services;
    });
  ...
  return (
    <>
      <Box
        ...
        <Button name="submitButton" type="submit" variant="success">
          Zatwierdź
        </Button>
      </Box>
    </>
